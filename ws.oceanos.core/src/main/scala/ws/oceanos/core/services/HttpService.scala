...
/*
class HttpService extends Actor with SprayActorLogging {
  ...
  def receive = {
    ...
    //case Context(ctx) => {log.info("setting context["+ctx+"]"); flow = ctx}
    ...
    case h @ HttpRequest(GET, Uri.Path("/hello"), _, _, _) =>
      val client = sender
      context.parent ? h onComplete {
        case Success(result) => client ! responsePresentation(result)
        case Failure(result) => client ! responsePresentation("Error:"+result)
      }
    case h @ HttpRequest(GET, Uri.Path("/beauty"), _, _, _) =>
      val client = sender
      context.parent ? h onComplete {
        case Success(result) => client ! responsePresentation(result)
        case Failure(result) => client ! responsePresentation("Error:"+result)
    ...
    case HttpRequest(GET, Uri.Path("/flow"), _, _, _) =>
      val client = sender
      //f ? Request(None) onSuccess {
        //case Response(v) => client ! responsePresentation(v)
      //}
  ...
  def responsePresentation(value: Any) = HttpResponse(
    entity = HttpEntity(`text/html`,
      <html>
        <body>
          <h1>OceanOS Response</h1>
          <h3>{value}</h3>
        </body>
      </html>.toString()
    )
